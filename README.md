...
В дальнейшем вместо вероятностных сценариев будем использовать рассчитанные выше коэффициенты ожидаемого изменения цены активов (т.е. будем считать задачу полностью детерминированной).
...
Значения `w_k` известны для всех трёх этапов:
...
Для задачи динамического программирования с конечным числом этапов (`N = 3`) и детерминированными переходами:

- **Состояние системы** на этапе `k`: `s_k ∈ S_k`, где `S_k` - множество возможных состояний на этапе `k`.

- **Управление (решение)** на этапе `k`: `x_k ∈ D_k(s_k)`, где `D_k(s_k)` - множество допустимых управлений в состоянии `s_k` на этапе `k`.

- **Функция перехода (уравнение состояния)**: `s_{k+1} = T_k(s_k, x_k)`. Она определяет, в какое состояние `s_{k+1}` мы перейдём из состояния `s_k`, применив управление `x_k`.

- **Критерий оптимальности**: Суммарный выигрыш (или его математическое ожидание в стохастическом случае) за все этапы. Здесь это `F = sum(s_N)`.

**Функция Беллмана (`V_k(s_k)`) - условная оптимальная доходность.**<br>
Она определяет максимальную итоговую стоимость портфеля, которую можно получить, начиная с этапа `k` из состояния `s_k` и применяя оптимальные управления на всех последующих этапах.

**Рекуррентное соотношение Беллмана (обратный проход):**<br>
Вычисления ведутся от последнего этапа к первому.

1. **Граничное условие** на последнем этапе (`k = N+1`, после всех событий):<br>
`V_{N+1}(s_{N+1}) = sum(s_{N+1})`<br>
Это просто итоговая стоимость всех активов.

2. **Основное рекуррентное соотношение** для этапов `k = N, N-1, ..., 1`:<br>
`V_k(s_k) = max_{x_k ∈ D_k(s_k)} [ V_{k+1}( T_k(s_k, x_k) ) ]`

    - Смысл: Оптимальная доходность из состояния `s_k` на этапе `k` равна максимальной из оптимальных доходностей, достижимых из всех состояний `s_{k+1}`, в которые можно перейти из `s_k` допустимым управлением `x_k`.

**Прямой проход:**

После того как функции `V_k(s_k)` и соответствующие оптимальные управления `x_k^*(s_k)` найдены для всех `k` в обратном проходе, мы выполняем прямой проход для нахождения оптимальной траектории при заданном начальном состоянии `s_1`:

1. Начинаем с `s_1` (известно).

2. Для `k = 1`: Применяем найденное оптимальное управление `x_1^*(s_1)`, вычисляем новое состояние `s_2 = T_1(s_1, x_1^*)`.

3. Для `k = 2`: Применяем `x_2^*(s_2)`, получаем `s_3 = T_2(s_2, x_2^*)`.

4. Для `k = 3`: Применяем `x_3^*(s_3)`, получаем `s_4 = T_3(s_3, x_3^*)`.

5. Итоговая стоимость равна `V_1(s_1)`.
